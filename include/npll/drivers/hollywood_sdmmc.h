...
//	struct sdmmc_task c_task;	/* task queue entry */
	u16	 	c_opcode;	/* SD or MMC command index */
	u32	 	c_arg;		/* SD/MMC command argument */
...
static __inline int
__bitfield(u32 *src, int start, int len)
{
	u8 *sp;
	u32 dst, mask;
	int shift, bs, bc;
